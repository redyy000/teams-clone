...
def test_not_already_pinned(setup_users):
    user1 = setup_users[0]
    user2 = setup_users[1]
...
def test_unpin_with_no_owner_permissions(setup_users):
    user1 = setup_users[0]
    user2 = setup_users[1]
    channel_response = requests.post(f"{config.url}channels/create/v2", json={
        "token": user1['token'],
...
def test_unpin_with_no_owner_permissions2(setup_users):
    user1 = setup_users[0]
    user2 = setup_users[1]
    dm_response = requests.post(f'{config.url}dm/create/v1', json={
        'token': user1['token'],
...
def test_unpin_success(setup_users):
    user1 = setup_users[0]
    channel_response = requests.post(f"{config.url}channels/create/v2", json={
        "token": user1['token'],
        "name": "general",
    ...
    assert messages_response.json()['messages'][0]['is_pinned'] == False

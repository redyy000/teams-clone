...
from src.other import is_valid_token, message_notification, is_channel_member
...
def standup_thread(token, channel, start):
    ...
    if len(standup["buffer"]) > 0:
        ...
        for text in standup["buffer"]:
            ...
            user_exists = next(
                (c for c in store["users"] if c["u_id"] == text["u_id"]), None)
        ...
        # Could delete notifications afterwards...
        # Removes tagging notifications if they exist
        for user in store['users']:
            tag = '@' + user['handle_str']
            # Check if tag in message
            # Check if member in channel
            is_member = is_channel_member(user['u_id'], channel["channel_id"])
            if tag in message and is_member:
                del user['notifications'][-1]
...
def standup_start_v1(token, channel_id, length):
    ...
    new_thread = threading.Timer(
        float(length), standup_thread, args=(token, curr_channel, start))
    new_thread.start()
